...
// Determine the source for the iframe based on media type
const videoSrc = computed(() => {
  if (props.item.media_type === 'tv') {
    return `https://vidsrc.me/embed/tv?tmdb=${props.item.id}&season=${props.item.season_number}&episode=${props.item.episode_number}`
  }
  return `https://vidsrc.me/embed/movie?tmdb=${props.item.id}`
})
</script>
  ...
  <div :key="item.id" relative class="aspect-ratio-3/2 lg:aspect-ratio-25/9 bg-black">
    ...
    <div class="iframe-container absolute top-0 left-0 right-0">
      <div class="player-container">
        ...
        <iframe
          v-if="isPlaying"
          :src="videoSrc" <!-- Bind video source based on media type -->
          class="video-iframe"
          frameborder="0"
